...
class Archive<T> where T : Individual {
    ...
    fun getNotCoveredTargetDescriptions(): Map<Int, String> {
        // 获取未覆盖的目标 ID
        val notCoveredTargets = notCoveredTargets()
    
        // 创建映射，将未覆盖的目标 ID 映射到其描述性 ID
        return notCoveredTargets.associateWith { idMapper.getDescriptiveId(it) }
    }
