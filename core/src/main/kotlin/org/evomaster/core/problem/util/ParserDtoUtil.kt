...
//        }
    ...
    fun setGeneBasedOnStringNew(gene: Gene, stringValue: String?):Boolean{
        val valueGene = ParamUtil.getValueGene(gene)

        // LoggingUtil.getInfoLogger().info(
        //     "*geneType (${valueGene::class.java.name}) stringValue ${stringValue} "
        // )
        ...
        if (stringValue != null && !stringValue.equals("null", ignoreCase = true)){
            when(valueGene){
                ...
                is StringGene -> valueGene.setFromStringValue(stringValue)
                ...
                is ArrayGene<*> -> {
                    ...
                    val tempValueGene = valueGene.copy()
                    
                    if (node.isArray){
                        try{
                            valueGene.killAllChildren()
                            node.run {
                            ...
                            valueGene.copyValueFrom(tempValueGene)
                            return false
                        }
                    }else{
                ...
                is ObjectGene -> {
                    ...
                    if (node.isObject){
                        ...
                        valueGene.fields.forEach { f->
                            // LoggingUtil.getInfoLogger().info("f is {} {}",f.name,f.getValueAsPrintableString())
                            ...
                            if(f is OptionalGene){
                                if (pdto == null)f.isActive = false
                            ...
                            objectNode.remove(f.name)
                        }
                        objectNode.fields().forEach { entry ->
                            LoggingUtil.getInfoLogger().info("Key: ${entry.key}, Value: ${entry.value.asText()}")
                            valueGene.isFixed = false
                            valueGene.addChild(OptionalGene(entry.key, StringGene(entry.key, value = entry.value.asText())).apply { doInitialize() })
                            valueGene.isFixed = true
                        }
                    }else{
                        throw IllegalStateException("stringValue ($stringValue) is not Object")
                    }
