...
import org.evomaster.core.problem.util.ParserDtoUtil.setGeneBasedOnStringNew
...
class RestCallAction(
    ...
    companion object {
        private val log: Logger = LoggerFactory.getLogger(RestCallAction::class.java)
    }
    ...
    fun getBody(): String? {
        val body = parameters.find { p -> p is BodyParam }
        if (body != null && body is BodyParam) {
            //LoggingUtil.getInfoLogger().info("Contenet type of body is {}", body.contentType())
            return body.gene.getValueAsPrintableString()
        }
        return null
    }

    fun setBody(value : String): Boolean{
        val body = parameters.find { p -> p is BodyParam }

        if (body != null && body is BodyParam) {
            try {
                return setGeneBasedOnStringNew(body.gene, value)
            } catch (e: Exception) {
                LoggingUtil.uniqueWarn(
                    log,
                    "*LLM for request, fail to mutate gene based on a given gene and an exception (${e.message}) is thrown"
                )
                return false
            }
        }
    ...
    fun getQuery(): String? {
        val querys = parameters.filterIsInstance<QueryParam>()
        if(querys.isEmpty()){
            return null
        }
        return querys.map {
                    val name = it.gene.getVariableName()
                    val value = it.gene.getValueAsRawString()
                    "$name=$value"
                }
                .joinToString("&")
    }

    fun setQuery(key : String, value : String): Boolean{
        if(key == ""||key == "{}"){
            var node : JsonNode
            try{
                node = ObjectMapper().readTree(value)
                // LoggingUtil.getInfoLogger().info("Node is {}", node)
            } catch (e: Exception) {
                return false
            }
            var result = false
            if (node.isObject){
                ...
                objectNode.fields().forEach { entry ->
                    // LoggingUtil.getInfoLogger().info("Path Key: ${entry.key}, Value: ${entry.value.asText()}")
                    val query =  parameters.find { p -> p is QueryParam && p.name == entry.key } 
                    if (query != null && query is QueryParam) {
                        try {
                            result = result or setGeneBasedOnStringNew(query.gene, entry.value.asText())
                            // LoggingUtil.getInfoLogger().info("Reslut: $result")
                        } catch (e: Exception) {
                            LoggingUtil.uniqueWarn(
                                log,
                                "*LLM for request, fail to mutate gene based on a given gene and an exception (${e.message}) is thrown"
                        ...
                        val gene = StringGene(entry.key, value = entry.value.asText()).apply { doInitialize() }
                        this.addParam(
                            QueryParam(entry.key,
                                OptionalGene(entry.key, gene).apply { doInitialize() })
                        )
                        result = true
                    }
                }
            ...
            val query =  parameters.find { p -> p is QueryParam && p.name == key } 

            if (query != null && query is QueryParam) {
                try {
                    if(query.gene is OptionalGene) query.gene.isActive = true
                    return setGeneBasedOnStringNew(query.gene, value)
                } catch (e: Exception) {
                    LoggingUtil.uniqueWarn(
                        log,
                        "*LLM for request, fail to mutate gene based on a given gene and an exception (${e.message}) is thrown"
                ...
                val gene = StringGene(key, value = value).apply { doInitialize() }
                this.addParam(
                    QueryParam(key,
                        OptionalGene(key, gene).apply { doInitialize() })
                )
                return true
            }
        }
    ...
    fun setPath(key : String, value : String): Boolean{
        if(key == ""||key == "{}"){
            var node : JsonNode
            try{
                node = ObjectMapper().readTree(value)
            ...
            if (node.isObject){
                ...
                objectNode.fields().forEach { entry ->
                    // LoggingUtil.getInfoLogger().info("Path Key: ${entry.key}, Value: ${entry.value.asText()}")
                    val path =  parameters.find { p -> p is PathParam && p.name == entry.key } 
                    if (path != null && path is PathParam) {
                        try {
                            result = result or setGeneBasedOnStringNew(path.gene, entry.value.asText())
                            // LoggingUtil.getInfoLogger().info("Reslut: $result")
                        } catch (e: Exception) {
                            LoggingUtil.uniqueWarn(
                                log,
            ...
            val path =  parameters.find { p -> p is PathParam && p.name == key } 

            if (path != null && path is PathParam) {
                try {
                    return setGeneBasedOnStringNew(path.gene, value)
                } catch (e: Exception) {
                    LoggingUtil.uniqueWarn(
                        log,
                        "*LLM for request, fail to mutate gene based on a given gene and an exception (${e.message}) is thrown"
    ...
    fun getPath(): String? {
        val paths = parameters.filterIsInstance<PathParam>()
        if(paths.isEmpty()){
            return null
        }
        return paths.filter { it.gene !is OptionalGene || it.gene.isActive }
                .map {
                    val name = it.gene.getVariableName()
                    val value = it.gene.getValueAsRawString()
                    "$name=$value"
    ...
    fun getHeader(): String? {
        val headers = parameters.filterIsInstance<HeaderParam>()
        if(headers.isEmpty()){
            return null
        }
        return headers.map {
                    val name = it.name
                    val value = it.getRawValue()
                    "$name=$value"
                }
                .joinToString("&") +"&"+ auth.headers.map{
                    val name = it.name
                    val value = it.value
                    "$name=$value"
                }
    ...
    fun setHeader(key : String, value : String): Boolean {
        if(key == ""||key == "{}"){
            var node : JsonNode
            try{
                node = ObjectMapper().readTree(value)
            ...
            if (node.isObject){
                ...
                objectNode.fields().forEach { entry ->
                    // LoggingUtil.getInfoLogger().info("Path Key: ${entry.key}, Value: ${entry.value.asText()}")
                    val header =  parameters.find { p -> p is HeaderParam && p.name == entry.key } 
                    if (header != null && header is HeaderParam) {
                        try {
                            result = result or setGeneBasedOnStringNew(header.gene, entry.value.asText())
                            // LoggingUtil.getInfoLogger().info("Reslut: $result")
                        } catch (e: Exception) {
                            LoggingUtil.uniqueWarn(
                                log,
                    ...
                    val headerauth =  auth.headers.find { p -> p is AuthenticationHeader && p.name == entry.key }
                    if (headerauth != null && headerauth is AuthenticationHeader) {
                        headerauth.value = entry.value.asText()
                        result = true
                    }
                }
                return result
            ...
            val header =  parameters.find { p -> p is HeaderParam && p.name == key } 

            if (header != null && header is HeaderParam) {
                try {
                    if(header.gene is OptionalGene) header.gene.isActive = true
                    return setGeneBasedOnStringNew(header.gene, value)
                } catch (e: Exception) {
                    LoggingUtil.uniqueWarn(
                        log,
                        "*LLM for request, fail to mutate gene based on a given gene and an exception (${e.message}) is thrown"
            ...
            val headerauth =  auth.headers.find { p -> p is AuthenticationHeader && p.name == key }
            if (headerauth != null && headerauth is AuthenticationHeader) {
                headerauth.value = value 
                return true
            }
